...
if(isset($_POST['placeOrderBtn'])){
    ...
    if($name == "" || $email == "" || $phone == "" || $pincode == "" || $address == ""){
        $_SESSION['message'] = "all field are require";
        header('Location: ../checkout.php');
        exit(0);
    }
    
    $userId = $_SESSION['auth_user']['user_id'];
    $query = "SELECT c.id as cid, c.prod_id, c.prod_qty, p.id as pid, p.name, p.image, p.selling_price FROM 
    carts c, products p WHERE c.prod_id=p.id AND c.user_id='$userId' ORDER BY c.id DESC ";
...
$totalprice += $citem['selling_price'] * $citem['prod_qty'];
    ...
    $tracking_no = "ankitcoder".rand(1111,9999).substr($phone,2);
   ...
   if($insert_query_run){
    ...
    $insert_items_query = "INSERT INTO order_items (order_id, prod_id, qty, price) VALUE ('$order_id','$prod_id','$prod_qty','$price')";
